...
def joint(x1, y1, x2, y2, x3, y3, angle, w1, w2):
	angle_rad = math.pi*angle/180.
	line1 = line(x1, y1, x2, y2)
	length = math.sqrt((x3-x2)*(x3-x2) + (y3-y2)*(y3-y2))
	x = x2 + length * math.cos(angle_rad)
        y = y2 + length * math.sin(angle_rad)
	line2 = line(x2, y2, x, y)
        return get_line_head(w1) + line1 + line_tail + ",\n" + get_line_head(w2) + line2 + line_tail
...
print json_head, joint(30.0, 60.0, 30.001, 60.0, 30.002, 60.0, alpha, 2, 4), json_tail
